  ...
  <div class="graph-panel">
    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <!-- Indicador de construção/simulação -->
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          ...
          {{ isSimulating ? 'Atualização em tempo real da memória GraphRAG' : 'Atualizando em tempo real...' }}
        </div>
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">Ainda há conteúdo sendo processado. Recomendamos atualizar o grafo manualmente em breve.</span>
      ...
      <div v-else class="graph-state">
        ...
        <p class="empty-text">Aguardando geração da ontologia...</p>
      </div>
    </div>
...
const wasSimulating = ref(false) // Rastrear se estava simulando anteriormente
...
// Monitorar mudanças em isSimulating, detectar término da simulação
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Transição de simulando para não simulando, exibir aviso de término
    showSimulationFinishedHint.value = true
  }
  wasSimulating.value = newValue
}, { immediate: true })

// Alternar estado expandido/recolhido do item de auto-loop
...
const closeDetailPanel = () => {
  ...
  expandedSelfLoops.value = new Set() // Redefinir estado de expansão
}
...
const renderGraph = () => {
  ...
  // Processar dados das arestas, calcular quantidade e índice entre o mesmo par de nós
  const edgePairCount = {}
  ...
  // Registrar o índice atual da aresta para cada par de nós
  const edgePairIndex = {}
  ...
  tempEdges.forEach(e => {
    ...
    // Verificar se a direção da aresta é consistente com a direção normalizada (UUID origem < UUID destino)
    const isReversed = e.source_node_uuid > e.target_node_uuid
    
    // Calcular curvatura: dispersar com múltiplas arestas, linha reta para aresta única
    let curvature = 0
    if (totalCount > 1) {
      // Distribuir curvatura uniformemente, garantindo distinção clara
      // O intervalo de curvatura aumenta com o número de arestas
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // Se a direção da aresta for oposta à direção normalizada, inverter curvatura
      // Isso garante que todas as arestas sejam distribuídas no mesmo referencial, sem sobreposição
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  // Simulação - ajustar espaçamento entre nós dinamicamente com base no número de arestas
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Ajustar distância dinamicamente com base no número de arestas entre o par de nós
      // Distância base 150, aumenta 40 para cada aresta adicional
    ...
    // Adicionar gravidade central para agrupar nós independentes na área central
  ...
  const getLinkPath = (d) => {
    ...
    if (d.isSelfLoop) {
      ...
      const loopRadius = 30
      // Partindo do lado direito do nó, fazendo uma volta
    ...
    // Calcular ponto de controle da curva - ajustar dinamicamente com base no número de arestas e distância
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // Deslocamento perpendicular à direção da linha, calculado proporcionalmente à distância
    // Quanto mais arestas, maior a proporção do deslocamento
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 // Base 25%, aumenta 5% para cada aresta adicional
  ...
  // Calcular ponto médio da curva (para posicionamento do rótulo)
  const getLinkMidpoint = (d) => {
    ...
    // Detectar auto-loop
    if (d.isSelfLoop) {
      // Posição do rótulo do auto-loop: lado direito do nó
      return { x: sx + 70, y: sy }
    }
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // Detectar se o arrasto realmente começou (movimento acima do limiar)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // Primeira detecção de arrasto real, reiniciar simulação
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Somente parar simulação gradualmente se houve arrasto real
        if (d._isDragging) {
    ...
    .on('click', (event, d) => {
      event.stopPropagation()
      // Redefinir estilos de todos os nós
...
// Monitorar interruptor de exibição dos rótulos das arestas
...
/* Estilos do aviso pós-simulação */
