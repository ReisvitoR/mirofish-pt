    ...
    <div class="main-split-layout">
      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="workflow-overview" v-if="agentLogs.length > 0 || reportOutline">
          ...
          <!-- Botão Próximo Passo - exibido após conclusão -->
          <button v-if="isComplete" class="next-step-btn" @click="goToInteraction">
            <span>Entrar em interação profunda</span>
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Conteúdo da seção gerado (conteúdo gerado, mas seção pode não estar concluída) -->
                  ...
                  <!-- Seção concluída (geração da seção concluída) -->
...
const toggleRawResult = (timestamp, event) => {
  // Salvar posição do botão em relação ao viewport
  ...
  // Após atualização do DOM, ajustar posição de scroll para manter o botão na mesma posição
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extrair dados estatísticos - formato "fatos de previsão relacionados: X"
    ...
    // Extrair subquestões - extração completa, sem limite
    ...
    // Extrair fatos-chave - extração completa, sem limite
    ...
    // Extrair entidades centrais - extração completa, com resumo e contagem de fatos
    ...
    // Extrair cadeias de relação - extração completa, sem limite
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extrair fatos válidos atuais - extração completa, sem limite
    ...
    // Extrair fatos históricos/expirados - extração completa, sem limite
    ...
    // Extrair entidades envolvidas - extração completa, sem limite
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // Extrair motivo da seleção dos entrevistados
    const reasonMatch = text.match(/### 采访对象选择理由\n([\s\S]*?)(?=\n---\n|\n### 采访实录)/)
    if (reasonMatch) {
      result.selectionReason = reasonMatch[1].trim()
    }
    
    // Analisar motivo de seleção de cada pessoa
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Ex: 1. **ex-aluno_345（index=1）**：como ex-aluno da universidade...
        ...
        // Ex: - selecionarPai_601（index 0）：como representante do grupo de pais...
        if (!headerMatch) {
        ...
        // Ex: - **pai_601（index 0）**：como representante do grupo de pais...
        if (!headerMatch) {
        ...
        if (name) {
          // Salvar motivo da pessoa anterior
          ...
          // Continuação do motivo (excluir parágrafo de conclusão)
          currentReason.push(line.trim())
        }
      }
      
      // Salvar motivo da última pessoa
      if (currentName && currentReason.length > 0) {
        reasons[currentName] = currentReason.join(' ').trim()
      }
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (nameRoleMatch) {
        ...
        // Definir motivo de seleção desta pessoa
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // Se a primeira pergunta tem "1." antes, tratamento especial
      ...
      if (answerMatch) {
        ...
        // Lógica de fallback de plataforma (compatibilidade: apenas uma plataforma)
        if (!twitterMatch && redditMatch) {
          // Somente resposta Reddit, copiar como exibição padrão apenas quando não for texto placeholder
          ...
          // Sem marcador de plataforma (formato antigo), usar como resposta completa
          interview.twitterAnswer = answerText
        }
      }
      
      // Extrair citações-chave (compatível com vários formatos de aspas)
      const quotesMatch = block.match(/\*\*关键引言:\*\*\n([\s\S]*?)(?=\n---|\n####|$)/)
      if (quotesMatch) {
        const quotesText = quotesMatch[1]
        // Priorizar formato > "text"
        let quoteMatches = quotesText.match(/> "([^"]+)"/g)
        // Fallback: formato > "text" ou > \u201Ctext\u201D (aspas chinesas)
        if (!quoteMatches) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // Extrair fatos relacionados - extração completa, sem limite
    ...
    // Tentar extrair informações de arestas
    ...
    // Tentar extrair informações de nós
  ...
  setup(props) {
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, 'Nenhuma cadeia de relação')
      ])
    ])
  }
}
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Verificar se é texto placeholder de plataforma
    ...
    // Tentar dividir respostas por número de pergunta
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // Suporte a dois formatos de numeração:
      // 1. "问题X：" ou "问题X:" (formato chinês "PerguntaX:", novo formato do backend)
      // 2. "1. " ou "\n1. " (número+ponto, compatibilidade com formato antigo)
      let matches = []
      let match

      // Priorizar formato "问题X：" (PerguntaX:)
      const cnPattern = /(?:^|[\r\n]+)问题(\d+)[：:]\s*/g
      ...
      // Se não encontrou numeração ou só encontrou uma, retornar tudo
      if (matches.length <= 1) {
      ...
      // Extrair cada parte pela numeração
      const parts = []
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Falha na divisão: primeira pergunta retorna resposta completa, demais retornam vazio
      return qIdx === 0 ? answer : ''
    }
    
    // Verificar se uma pergunta tem respostas de duas plataformas (filtrar placeholder)
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Ambas as plataformas têm respostas reais (não placeholder) e conteúdos diferentes
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Botão de alternância entre plataformas (exibido apenas quando há respostas reais em ambas as plataformas)
                  ...
                  // Botão Expandir/Recolher (placeholder não exibido)
...
const activeStep = computed(() => {
  ...
  // Se não há ativa, retornar última etapa concluída
  const doneSteps = steps.filter(s => s.status === 'done')
  if (doneSteps.length > 0) return doneSteps[doneSteps.length - 1]
  
  // Caso contrário, retornar primeira etapa
...
const renderMarkdown = (content) => {
  ...
  // Remover título H2 do início (## xxx), pois o título da seção já é exibido externamente
  ...
  // Limpar espaços entre itens de lista
  html = html.replace(/<\/li>\s+<li/g, '</li><li')
  // Limpar espaços após tag de abertura de lista
  ...
  // Processar quebras de linha - linhas vazias viram separação de parágrafo, quebra simples vira <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Limpar tags <br> antes/depois de elementos de bloco
  html = html.replace(/<br>\s*(<ul|<ol|<blockquote)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>)\s*<br>/g, '$1')
  // Limpar <p><br> seguido de elementos de bloco (causado por linhas vazias extras)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Limpar tags <br> consecutivas
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Limpar <br> antes da tag de parágrafo após elemento de bloco
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Corrigir numeração de listas ordenadas não contínuas: quando <ol> de item único é separado por conteúdo, manter numeração crescente
...
const getLogLevelClass = (log) => {
  ...
  // INFO usa cor padrão, não marcado como success
  return ''
}
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          // section_complete - Geração de seção concluída
          if (log.action === 'section_complete') {
            if (log.details?.content) {
              generatedSections.value[log.section_index] = log.details.content
              // Expandir automaticamente seção recém-gerada
          ...
          if (log.action === 'report_complete') {
            ...
            currentSectionIndex.value = null  // Garantir limpeza do estado loading
            emit('update-status', 'completed')
            stopPolling()
            // Lógica de scroll unificada no nextTick após o fim do loop
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // Se tarefa concluída, scroll para topo; senão scroll para fundo seguindo logs mais recentes
            if (isComplete.value) {
...
// Extrair conteúdo da resposta final - extrair conteúdo de seção da resposta LLM
const extractFinalContent = (response) => {
  ...
  // Tentar encontrar conteúdo após Final Answer: (suporte a múltiplos formatos)
  ...
  // Tentar encontrar conteúdo após 最终答案 (Resposta Final):
  const chineseFinalMatch = response.match(/最终答案[:：]\s*\n*([\s\S]*)$/i)
  if (chineseFinalMatch) {
    return chineseFinalMatch[1].trim()
  }
  
  // Se começa com ## ou # ou >, provavelmente é conteúdo markdown direto
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }
  
  // Se conteúdo é longo e contém formato markdown, tentar remover processo de pensamento
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Remover processo de pensamento que começa com Thought:
