          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- Fallback genérico: tipo desconhecido ou tem content mas não foi tratado acima -->
...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // Máximo de rodadas passado do Step2
  minutesPerRound: {
    type: Number,
...
const runStatus = ref({})
const allActions = ref([]) // Todas as ações (acumulativas)
const actionIds = ref(new Set()) // Conjunto de IDs de ações para deduplicação
const scrollContainer = ref(null)

// Computed
// Exibir ações em ordem cronológica (mais recentes no final, ou seja, na parte inferior)
...
// Formatar tempo decorrido da simulação (calculado com base nas rodadas e minutos por rodada)
...
// Tempo decorrido na plataforma Twitter
const twitterElapsedTime = computed(() => {
  return formatElapsedTime(runStatus.value.twitter_current_round || 0)
})

// Tempo decorrido na plataforma Reddit
...
const resetAllState = () => {
  ...
  stopPolling()  // Parar polling que possa existir
}
...
const doStartSimulation = async () => {
  ...
  // Primeiro resetar todos os estados, garantindo que não seja afetado pela simulação anterior
  resetAllState()
...
// Rastrear a última rodada de cada plataforma, para detectar mudanças e registrar logs
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detectar mudanças de rodada em cada plataforma e registrar logs
      ...
      // Verificar via status twitter_completed e reddit_completed
      const platformsCompleted = checkPlatformsCompleted(data)
      
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ Simulação em todas as plataformas encerrada')
        }
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Não rolar automaticamente, permitir que o usuário visualize a linha do tempo livremente
      // Novas ações serão adicionadas na parte inferior
    }
  } catch (err) {
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('Solicitação de geração de relatório enviada, aguarde...')
    return
  }
...
onMounted(() => {
  addLog('Step3 Inicialização da execução de simulação')
  if (props.simulationId) {
    doStartSimulation()
  }
})
