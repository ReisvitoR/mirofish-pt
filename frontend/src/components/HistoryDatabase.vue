<template>
  ...
  >
    <!-- Decoração de fundo: grade técnica (visível apenas quando há projetos) -->
    ...
    <!-- Container de cards (visível apenas quando há projetos) -->
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        <!-- Cabeçalho do card: simulation_id e status de funcionalidades -->
        ...
        <div class="card-files-wrapper">
          ...
          <div class="files-list" v-if="project.files && project.files.length > 0">
            ...
            <!-- Se houver mais arquivos, exibir indicação -->
        ...
        <!-- Descrição do card (exibição completa do requisito de simulação) -->
    ...
    <!-- Modal de detalhes do histórico de reprodução -->
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <!-- Divisor de reprodução da simulação -->
            ...
            <!-- Aviso de reprodução indisponível -->
            <div class="modal-playback-hint">
              <span class="hint-text">Step3 "Iniciar Simulação" e Step5 "Interação Avançada" devem ser iniciados em tempo real e não suportam reprodução do histórico</span>
            </div>
          </div>
        </div>
      </Transition>
...
let observer = null
let isAnimating = false  // Trava de animação para evitar flickering
let expandDebounceTimer = null  // Timer de debounce
let pendingState = null  // Registra o estado alvo pendente

// Configuração do layout dos cards - ajustado para proporção mais larga
const CARDS_PER_ROW = 4
const CARD_WIDTH = 280  
const CARD_HEIGHT = 280 
const CARD_GAP = 24

// Cálculo dinâmico do estilo de altura do container
const containerStyle = computed(() => {
  ...
  // Estado expandido: calcula altura dinamicamente conforme a quantidade de cards
  ...
  // Calcula a altura necessária: linhas × altura do card + (linhas-1) × espaçamento + margem inferior
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calcula quantidade de cards na linha atual, garantindo centralização
    ...
    const x = offset * 35
    // Ajusta posição inicial, próximo ao título mas mantendo espaçamento adequado
...
// Obter classe de estilo conforme o progresso das rodadas
...
// Formatar hora (exibir horas:minutos)
...
// Formatar exibição das rodadas (rodada atual/total de rodadas)
...
// Obter tipo de arquivo (para estilo)
...
// Truncar nome do arquivo (preservar extensão)
...
// Navegar para a página do relatório de análise (Report)
const goToReport = () => {
...
const loadHistory = async () => {
  try {
    ...
    console.error('Falha ao carregar histórico de projetos:', error)
    projects.value = []
  } finally {
    loading.value = false
  }
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Atualizar estado alvo pendente (registrar o estado mais recente independente da animação)
        pendingState = shouldExpand
        
        // Limpar timer de debounce anterior (nova intenção de rolagem sobrescreve a anterior)
        if (expandDebounceTimer) {
          clearTimeout(expandDebounceTimer)
          expandDebounceTimer = null
        }
        
        // Se estiver animando, apenas registrar o estado e processar após a animação
        if (isAnimating) return
        
        // Se o estado alvo for igual ao estado atual, não é necessário processar
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }
        
        // Usar debounce para atrasar a troca de estado e evitar flickering rápido
        // Expandir com atraso curto (50ms), recolher com atraso longo (200ms) para maior estabilidade
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Verificar se o estado pendente ainda precisa ser executado (pode ter sido sobrescrito por rolagem posterior)
          ...
          setTimeout(() => {
            ...
            // Após animação, verificar se há novo estado pendente
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Atrasar um pouco antes de executar, evitando troca muito rápida
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // Ajustar rootMargin, encolher a parte inferior do viewport para cima, necessitando mais rolagem para expandir
...
// Observar mudanças de rota, recarregar dados ao retornar à página inicial
...
onMounted(async () => {
  // Garantir que o DOM foi renderizado antes de carregar os dados
  await nextTick()
  await loadHistory()
  
  // Inicializar observador após renderização do DOM
  setTimeout(() => {
    initObserver()
  }, 100)
})

// Se usar keep-alive, recarregar dados quando o componente for ativado
onActivated(() => {
  loadHistory()
})
...
/* Grade quadrada com espaçamento fixo usando padrão de fundo CSS */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* Posicionamento a partir do canto superior esquerdo; expansão apenas na parte inferior */
...
.cards-container {
  ...
  /* min-height calculado dinamicamente pelo JS, adaptável à quantidade de cards */
}
...
/* Grupo de ícones de status de funcionalidades */
.card-status-icons {
  display: flex;
  align-items: center;
  gap: 6px;
...
/* Exibição do progresso das rodadas */
.card-progress {
  display: flex;
  align-items: center;
  gap: 6px;
...
/* Estilo minimalista do rótulo de arquivo */
.file-tag {
  display: inline-flex;
  align-items: center;
  justify-content: center;
...
/* Cores de status do progresso - rodapé */
