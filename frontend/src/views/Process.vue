<template>
  <div class="process-page">
    ...
    <div class="main-content">
      <!-- Painel esquerdo: Grafo de conhecimento em tempo real -->
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        ...
        <div class="graph-container" ref="graphContainer">
          <!-- Visualização do grafo (exibe sempre que houver dados) -->
          <div v-if="graphData" class="graph-view">
            <svg ref="graphSvg" class="graph-svg"></svg>
            <!-- Dica de construção em andamento -->
          ...
          <div v-else-if="currentPhase === 1 && !graphData" class="graph-waiting">
            ...
            <p class="waiting-hint">Dados serão exibidos em breve...</p>
          </div>
      ...
      <!-- Lado direito: Detalhes do fluxo de construção -->
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  Após o upload dos documentos, o LLM analisa o conteúdo e gera automaticamente a estrutura de ontologia adequada para simulação de opinião (tipos de entidade + tipos de relação)
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">Descrição da API</div>
                <div class="detail-content">
                  Baseado na ontologia gerada, fragmenta documentos e chama a API Zep para construir o grafo de conhecimento, extraindo entidades e relações
                </div>
              </div>
              
              <!-- Aguardando conclusão da ontologia -->
              <div class="detail-section waiting-state" v-if="currentPhase < 1">
                <div class="waiting-hint">Aguardando conclusão da ontologia...</div>
              </div>
...
// ID do projeto atual (pode mudar de 'new' para ID real)
...
const ontologyProgress = ref(null) // Progresso da geração de ontologia
const currentPhase = ref(-1) // -1: Upload, 0: Gerando ontologia, 1: Construindo grafo, 2: Concluído
...
const statusText = computed(() => {
  if (error.value) return 'Construção falhou'
...
const goToNextStep = () => {
  ...
  alert('Funcionalidade de configuração do ambiente em desenvolvimento...')
}
...
// Inicialização - processar novo projeto ou carregar existente
...
// Processar novo projeto - chamar API de ontology/generate
const handleNewProject = async () => {
  const pending = getPendingUpload()
  
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'Nenhum arquivo pendente, retorne à página inicial'
    loading.value = false
    return
  }
  ...
  try {
    loading.value = true
    currentPhase.value = 0 // Fase de geração de ontologia
    ...
    error.value = 'Falha na inicialização do projeto: ' + (err.message || 'erro desconhecido')
  } finally {
    loading.value = false
  }
}

// Carregar dados de um projeto existente
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      buildProgress.value.message = 'Tarefa de construção do grafo iniciada...'
      ...
      error.value = response.error || 'Falha ao iniciar construção do grafo'
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = 'Falha ao iniciar construção do grafo: ' + (err.message || 'erro desconhecido')
    buildProgress.value = null
  }
}
...
const fetchGraphData = async () => {
  try {
    // Primeiro obter info do projeto para pegar o graph_id
    ...
    if (projectResponse.success && projectResponse.data.graph_id) {
      ...
      if (graphResponse.success && graphResponse.data) {
        ...
        // Atualizar renderização quando dados mudam
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        ...
        // Atualizar progresso para estado concluído
        buildProgress.value = {
          progress: 100,
...
const renderGraph = () => {
  ...
  // Criar mapa de nós para busca de nomes
  const nodeMap = {}
  nodesData.forEach(n => {
    nodeMap[n.uuid] = n
  })
  ...
  // Desenhar arestas (inclui linha transparente larga clicável)
  ...
  // Clicar na área vazia para fechar painel de detalhes
  svg.on('click', () => {
    closeDetailPanel()
  })
  
  simulation.on('tick', () => {
    // Atualizar posição de todas as arestas (inclui linhas visíveis e área de clique transparente)
