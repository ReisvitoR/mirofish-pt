...
Gerencia simulações paralelas nas plataformas Twitter e Reddit
Utiliza scripts predefinidos + geração inteligente de parâmetros de configuração via LLM
"""
...
class SimulationManager:
    ...
    3. Gerar parâmetros de configuração de simulação inteligentemente via LLM
    ...
    def _load_simulation_state(self, simulation_id: str) -> Optional[SimulationState]:
        """Carregar estado da simulação a partir do arquivo"""
    ...
    def create_simulation(
        ...
        Args:
            ...
            enable_twitter: Habilitar simulação Twitter
            enable_reddit: Habilitar simulação Reddit
    ...
    def prepare_simulation(
        ...
        Etapas:
        1. Ler e filtrar entidades do grafo Zep
        2. Gerar OASIS Agent Profile para cada entidade (aprimoramento LLM opcional, com suporte a paralelismo)
        3. Gerar parâmetros de configuração de simulação inteligentemente via LLM (tempo, atividade, frequência de postagem, etc.)
        ...
        Args:
            ...
            simulation_requirement: Descrição dos requisitos da simulação (para geração de configuração via LLM)
            ...
            use_llm_for_profiles: Usar LLM para gerar perfis detalhados
        ...
        if not state:
            raise ValueError(f"Simulação não encontrada: {simulation_id}")
        ...
        try:
            ...
            # Passar graph_id para habilitar busca no Zep e obter contexto mais rico
            ...
            # Definir caminho do arquivo para salvamento em tempo real (priorizar formato JSON do Reddit)
            ...
            profiles = generator.generate_profiles_from_entities(
                ...
                graph_id=state.graph_id,  # Passar graph_id para busca no Zep
                parallel_count=parallel_profile_count,  # Quantidade de geração em paralelo
                realtime_output_path=realtime_output_path,  # Caminho para salvamento em tempo real
            ...
            # Reddit já foi salvo em tempo real durante a geração, salvando novamente para garantir integridade
            if progress_callback:
                progress_callback(
            ...
            if state.enable_twitter:
                # Twitter usa formato CSV! Isso é um requisito do OASIS
                generator.save_profiles(
                    profiles=profiles,
            ...
            # ========== Etapa 3: Geração inteligente de configuração de simulação via LLM ==========
            if progress_callback:
                progress_callback(
                    "generating_config", 0, 
    ...
    def list_simulations(self, project_id: Optional[str] = None) -> List[SimulationState]:
        ...
        if os.path.exists(self.SIMULATION_DATA_DIR):
            for sim_id in os.listdir(self.SIMULATION_DATA_DIR):
                # Ignorar arquivos ocultos (como .DS_Store) e arquivos que não são diretórios
    ...
    def get_run_instructions(self, simulation_id: str) -> Dict[str, str]:
        ...
        return {
            ...
            "instructions": (
                ...
                f"2. Executar simulação (scripts localizados em {scripts_dir}):\n"
