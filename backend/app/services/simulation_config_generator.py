...
Utiliza LLM para gerar automaticamente parâmetros detalhados de simulação com base nos requisitos,
conteúdo dos documentos e informações do grafo de conhecimento.
Totalmente automatizado, sem necessidade de configuração manual de parâmetros.

Estratégia de geração em etapas para evitar falhas por conteúdo excessivamente longo:
...
CHINA_TIMEZONE_CONFIG = {
    # Período da madrugada (quase nenhuma atividade)
    ...
    # Período noturno (atividade em declínio)
    "night_hours": [23],
...
class AgentActivityConfig:
    ...
    # Frequência de postagens (número esperado de postagens por hora)
    ...
    # Velocidade de resposta (atraso de reação a eventos em alta, unidade: minutos simulados)
...
class TimeSimulationConfig:
    ...
    # Duração total da simulação (em horas simuladas)
    total_simulation_hours: int = 72  # Padrão: 72 horas simuladas (3 dias)
    
    # Tempo por rodada (minutos simulados) - padrão 60 minutos (1 hora), acelerando o fluxo do tempo
    ...
    # Horário de pico (19-22h, horário mais ativo dos chineses)
    peak_hours: List[int] = field(default_factory=lambda: [19, 20, 21, 22])
    peak_activity_multiplier: float = 1.5
    
    # Horário de baixa atividade (0-5h, quase nenhuma atividade)
...
class EventConfig:
    ...
    # Direção da narrativa de opinião
...
class PlatformConfig:
    ...
    # Limiar de viralização (quantidade de interações para acionar a disseminação)
...
class SimulationParameters:
    ...
    # Lista de configurações de Agents
...
class SimulationConfigGenerator:
    ...
    Utiliza LLM para analisar requisitos de simulação, conteúdo de documentos e informações de entidades do grafo,
    ...
    # Comprimento de truncamento do contexto por etapa (em caracteres)
    ...
    def generate_config(
        ...
        """
        Gera configuração completa de simulação de forma inteligente (geração em etapas)
        ...
        Args:
            ...
            entities: Lista filtrada de entidades
        ...
        total_steps = 3 + num_batches  # Config. de tempo + Config. de eventos + N lotes de Agent + Config. de plataforma
        current_step = 0
        ...
        report_progress(2, "Gerando configuração de eventos e tópicos em alta...")
        ...
        for batch_idx in range(num_batches):
            ...
            report_progress(
                3 + batch_idx,
                f"Gerando configuração de Agents ({start_idx + 1}-{end_idx}/{len(entities)})..."
            )
        ...
        reasoning_parts.append(f"Configuração de Agents: {len(all_agent_configs)} gerados com sucesso")
        
        # ========== Atribuir Agent publicador às postagens iniciais ==========
        logger.info("Atribuindo Agents publicadores adequados às postagens iniciais...")
        event_config = self._assign_initial_post_agents(event_config, all_agent_configs)
        assigned_count = len([p for p in event_config.initial_posts if p.get("poster_agent_id") is not None])
        reasoning_parts.append(f"Atribuição de postagens iniciais: {assigned_count} postagens atribuídas a publicadores")
    ...
    def _build_context(
        ...
        """Construir contexto do LLM, truncado ao comprimento máximo"""
    ...
    def _call_llm_with_retry(self, prompt: str, system_prompt: str) -> Dict[str, Any]:
        """Chamada LLM com retry, incluindo lógica de reparo de JSON"""
        import re
        ...
        for attempt in range(max_attempts):
            try:
                ...
                try:
                    ...
                    logger.warning(f"Falha ao analisar JSON (attempt {attempt+1}): {str(e)[:80]}")
    ...
    def _generate_time_config(self, context: str, num_entities: int) -> Dict[str, Any]:
        ...
        # Usar comprimento de truncamento de contexto configurado
...
### Princípios básicos (apenas como referência, ajuste de acordo com o evento específico e grupo participante):
- O grupo de usuários é chinês, deve seguir os hábitos de horário de Pequim
- Das 0-5h quase nenhuma atividade (coeficiente de atividade 0.05)
- Das 6-8h gradualmente ativo (coeficiente de atividade 0.4)
- Horário de trabalho 9-18h atividade moderada (coeficiente de atividade 0.7)
- Das 19-22h é horário de pico (coeficiente de atividade 1.5)
- Após 23h atividade diminui (coeficiente de atividade 0.5)
- Regra geral: madrugada baixa atividade, manhã crescente, trabalho moderado, noite pico
- **Importante**: os valores de exemplo abaixo são apenas referência, ajuste os horários conforme a natureza do evento e características do grupo participante
  - Exemplo: pico de estudantes pode ser 21-23h; mídia ativa o dia todo; órgãos oficiais apenas no horário de trabalho
  - Exemplo: notícias urgentes podem gerar discussão mesmo na madrugada, off_peak_hours pode ser reduzido

### Retorne em formato JSON (sem markdown)
...
- total_simulation_hours (int): Duração total da simulação, 24-168 horas, eventos urgentes curtos, tópicos contínuos longos
- minutes_per_round (int): Duração por rodada, 30-120 minutos, recomendado 60 minutos
- agents_per_hour_min (int): Número mínimo de Agents ativados por hora (faixa: 1-{max_agents_allowed})
- agents_per_hour_max (int): Número máximo de Agents ativados por hora (faixa: 1-{max_agents_allowed})
- peak_hours (array de int): Horários de pico, ajuste conforme grupo participante do evento
...
- reasoning (string): Breve explicação do motivo desta configuração"""
        ...
        system_prompt = "Você é um especialista em simulação de mídias sociais. Retorne em formato JSON puro, a configuração de tempo deve seguir os hábitos de horário dos chineses."
        
        try:
            return self._call_llm_with_retry(prompt, system_prompt)
        except Exception as e:
            logger.warning(f"Falha na geração LLM da configuração de tempo: {e}, usando configuração padrão")
    ...
    def _parse_time_config(self, result: Dict[str, Any], num_entities: int) -> TimeSimulationConfig:
        """Analisar resultado da configuração de tempo e validar que agents_per_hour não exceda o total de agents"""
...
- Extrair palavras-chave de tópicos em alta
- Descrever a direção de desenvolvimento da opinião pública
- Elaborar conteúdo das postagens iniciais, **cada postagem deve especificar poster_type (tipo do publicador)**

**Importante**: poster_type deve ser selecionado dos "tipos de entidade disponíveis" acima, para que as postagens iniciais sejam atribuídas ao Agent adequado.
Exemplo: declarações oficiais devem ser publicadas pelo tipo Official/University, notícias pelo MediaOutlet, opiniões de estudantes pelo Student.
...
{{
    ...
    "initial_posts": [
        {{"content": "conteúdo da postagem", "poster_type": "tipo de entidade (deve ser selecionado dos tipos disponíveis)"}},
        ...
    ],
    "reasoning": "<breve explicação>"
}}"""

        system_prompt = "Você é um especialista em análise de opinião pública. Retorne em formato JSON puro. Atenção: poster_type deve corresponder exatamente aos tipos de entidade disponíveis."
        
        try:
            return self._call_llm_with_retry(prompt, system_prompt)
        except Exception as e:
            logger.warning(f"Falha na geração LLM da configuração de eventos: {e}, usando configuração padrão")
            return {
                "hot_topics": [],
                "narrative_direction": "",
                "initial_posts": [],
    ...
    def _assign_initial_post_agents(
        ...
        """
        Atribuir Agent publicador adequado às postagens iniciais
        
        Combinar o agent_id mais adequado com base no poster_type de cada postagem
        """
        ...
        # Tabela de mapeamento de tipos (tratar diferentes formatos que o LLM pode gerar)
        type_aliases = {
        ...
        # Registrar índice de agent usado por tipo, evitando reutilizar o mesmo agent
        ...
        for post in event_config.initial_posts:
            ...
            # 3. Se ainda não encontrado, usar o agent com maior influência
    ...
    def _generate_agent_configs_batch(
        ...
        # Construir informações de entidade (usar comprimento de resumo configurado)
        entity_list = []
        ...
        prompt = f"""Com base nas seguintes informações, gere a configuração de atividade em mídias sociais para cada entidade.
...
## Tarefa
Gere a configuração de atividade para cada entidade, observe:
- **Horários seguem hábitos chineses**: das 0-5h quase sem atividade, 19-22h mais ativo
...
{{
    "agent_configs": [
        {{
            "agent_id": <deve corresponder à entrada>,
            "activity_level": <0.0-1.0>,
            "posts_per_hour": <frequência de postagens>,
        ...
        system_prompt = "Você é um especialista em análise de comportamento em mídias sociais. Retorne JSON puro, a configuração deve seguir os hábitos de horário dos chineses."
        
        try:
            result = self._call_llm_with_retry(prompt, system_prompt)
            llm_configs = {cfg["agent_id"]: cfg for cfg in result.get("agent_configs", [])}
        except Exception as e:
            logger.warning(f"Falha na geração LLM do lote de configuração de Agents: {e}, usando geração por regras")
            llm_configs = {}
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        if entity_type in ["university", "governmentagency", "ngo"]:
            # Órgãos oficiais: atividade no horário de trabalho, baixa frequência, alta influência
            return {
                "activity_level": 0.2,
                "posts_per_hour": 0.1,
