"""
Serviço de construção de grafo
Interface 2: Construção de Standalone Graph usando a API Zep
"""
...
class GraphBuilderService:
    """
    Serviço de construção de grafo
    Responsável por chamar a API Zep para construir o grafo de conhecimento
    """
    ...
    def build_graph_async(
        ...
        """
        Construção assíncrona do grafo
        ...
        Args:
            ...
            chunk_overlap: Tamanho da sobreposição entre blocos
        ...
        # Executar construção em thread de segundo plano
    ...
    def set_ontology(self, graph_id: str, ontology: Dict[str, Any]):
        """Definir ontologia do grafo (método público)"""
        import warnings
        ...
        # Este uso é exigido pelo Zep SDK; o aviso vem da criação dinâmica de classes e pode ser ignorado com segurança
        ...
        def safe_attr_name(attr_name: str) -> str:
            """Converter nomes reservados para nomes seguros"""
        ...
        for entity_def in ontology.get("entity_types", []):
            ...
            for attr_def in entity_def.get("attributes", []):
                ...
                # A API Zep exige description no Field, isto é obrigatório
        ...
        # Chamar API Zep para definir a ontologia
    ...
    def add_text_batches(
        ...
        """Adicionar texto ao grafo em lotes, retornando a lista de uuids de todos os episodes"""
        episode_uuids = []
        total_chunks = len(chunks)
        ...
        for i in range(0, total_chunks, batch_size):
            ...
            try:
                ...
                # Coletar uuids dos episodes retornados
    ...
    def _wait_for_episodes(
        ...
        """Aguardar a conclusão do processamento de todos os episodes (consultando o status processed de cada episode)"""
        if not episode_uuids:
            if progress_callback:
                progress_callback("Nenhuma espera necessária (sem episodes)", 1.0)
            return
        ...
        if progress_callback:
            progress_callback(f"Aguardando processamento de {total_episodes} blocos de texto...", 0)
        ...
        while pending_episodes:
            ...
            # Verificar o status de processamento de cada episode
            for ep_uuid in list(pending_episodes):
                try:
                    ...
                    # Ignorar erros de consulta individuais, continuar
                    pass
            
            elapsed = int(time.time() - start_time)
            if progress_callback:
                progress_callback(
                    f"Processando no Zep... {completed_count}/{total_episodes} concluídos, {len(pending_episodes)} pendentes ({elapsed}s)",
