...
Atualiza dinamicamente as atividades dos Agentes da simulação no grafo Zep
"""
...
@dataclass
class AgentActivity:
    """Registro de atividade do Agente"""
    ...
    def to_episode_text(self) -> str:
        """
        Converte a atividade em descrição textual para envio ao Zep
        
        Utiliza formato de linguagem natural para que o Zep possa extrair entidades e relações
        Não adiciona prefixo de simulação para evitar confundir a atualização do grafo
        """
        ...
        # Retorna diretamente no formato "nome do agente: descrição da atividade", sem prefixo de simulação
    ...
    def _describe_like_post(self) -> str:
        """Curtir postagem - inclui texto original e informações do autor"""
        post_content = self.action_args.get("post_content", "")
        post_author = self.action_args.get("post_author_name", "")
        
        if post_content and post_author:
            return f"curtiu a postagem de {post_author}: 「{post_content}」"
        elif post_content:
    ...
    def _describe_dislike_post(self) -> str:
        """Não curtir postagem - inclui texto original e informações do autor"""
        post_content = self.action_args.get("post_content", "")
        post_author = self.action_args.get("post_author_name", "")
        
        if post_content and post_author:
            return f"não curtiu a postagem de {post_author}: 「{post_content}」"
        elif post_content:
            return f"não curtiu uma postagem: 「{post_content}」"
        elif post_author:
            return f"não curtiu uma postagem de {post_author}"
        return "não curtiu uma postagem"
    
    def _describe_repost(self) -> str:
        """Repostagem - inclui conteúdo original e informações do autor"""
        original_content = self.action_args.get("original_content", "")
        original_author = self.action_args.get("original_author_name", "")
        
        if original_content and original_author:
            return f"repostou a postagem de {original_author}: 「{original_content}」"
        elif original_content:
            return f"repostou uma postagem: 「{original_content}」"
        elif original_author:
            return f"repostou uma postagem de {original_author}"
        return "repostou uma postagem"
    
    def _describe_quote_post(self) -> str:
        """Citação de postagem - inclui conteúdo original, informações do autor e comentário da citação"""
        original_content = self.action_args.get("original_content", "")
        original_author = self.action_args.get("original_author_name", "")
        ...
        base = ""
        if original_content and original_author:
            base = f"citou a postagem de {original_author}: 「{original_content}」"
        elif original_content:
    ...
    def _describe_create_comment(self) -> str:
        """Publicar comentário - inclui conteúdo do comentário e informações da postagem comentada"""
        content = self.action_args.get("content", "")
        post_content = self.action_args.get("post_content", "")
        post_author = self.action_args.get("post_author_name", "")
        ...
        if content:
            if post_content and post_author:
                ...
                return f"comentou na postagem de {post_author}: 「{content}」"
    ...
    def _describe_dislike_comment(self) -> str:
        """Não curtir comentário - inclui conteúdo do comentário e informações do autor"""
        comment_content = self.action_args.get("comment_content", "")
        comment_author = self.action_args.get("comment_author_name", "")
        
        if comment_content and comment_author:
            return f"não curtiu o comentário de {comment_author}: 「{comment_content}」"
        elif comment_content:
            return f"não curtiu um comentário: 「{comment_content}」"
        elif comment_author:
            return f"não curtiu um comentário de {comment_author}"
        return "não curtiu um comentário"
    
    def _describe_search(self) -> str:
        """Pesquisar postagens - inclui palavras-chave da pesquisa"""
        query = self.action_args.get("query", "") or self.action_args.get("keyword", "")
        return f"pesquisou 「{query}」" if query else "realizou uma pesquisa"
    
    def _describe_search_user(self) -> str:
        """Pesquisar usuários - inclui palavras-chave da pesquisa"""
...
class ZepGraphMemoryUpdater:
    ...
    Monitora o arquivo de log de ações da simulação, atualizando atividades dos agentes no grafo Zep em tempo real.
    Agrupa por plataforma e envia em lote ao Zep a cada BATCH_SIZE atividades acumuladas.
    
    Todos os comportamentos significativos são atualizados no Zep, e action_args contém informações contextuais completas:
    - Texto original de postagens curtidas/não curtidas
    - Texto original de postagens repostadas/citadas
    - Nome de usuários seguidos/silenciados
    - Texto original de comentários curtidos/não curtidos
    """
    ...
    def add_activity(self, activity: AgentActivity):
        """
        Adiciona uma atividade de agente à fila
        ...
        - SEARCH_USER (pesquisar usuários)
        - LIKE_POST/DISLIKE_POST (curtir/não curtir postagem)
        - REPOST (repostar)
        - FOLLOW (seguir)
        - MUTE (silenciar)
        - LIKE_COMMENT/DISLIKE_COMMENT (curtir/não curtir comentário)
        
        action_args contém informações contextuais completas (como texto original da postagem, nome de usuário, etc.).
        
        Args:
            activity: Registro de atividade do Agente
        """
    ...
    def add_activity_from_dict(self, data: Dict[str, Any], platform: str):
        """
        Adiciona atividade a partir de dados do dicionário
    ...
    def _worker_loop(self):
        """Loop de trabalho em segundo plano - envia atividades ao Zep em lote por plataforma"""
    ...
    def _send_batch_activities(self, activities: List[AgentActivity], platform: str):
        ...
        Args:
            activities: Lista de atividades dos Agentes
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                if attempt < self.MAX_RETRIES - 1:
                    ...
                    logger.error(f"Falha ao enviar lote ao Zep após {self.MAX_RETRIES} tentativas: {e}")
                    self._failed_count += 1
    
    def _flush_remaining(self):
        """Envia as atividades restantes da fila e do buffer"""
        # Primeiro processa as atividades restantes da fila, adicionando ao buffer
        ...
        # Em seguida envia as atividades restantes do buffer de cada plataforma (mesmo que não atinja BATCH_SIZE)
        with self._buffer_lock:
...
class ZepGraphMemoryManager:
    """
    Gerencia atualizadores de memória do grafo Zep para múltiplas simulações
    ...
    def create_updater(cls, simulation_id: str, graph_id: str) -> ZepGraphMemoryUpdater:
        ...
        """
        with cls._lock:
            # Se já existir, para o antigo primeiro
    ...
    # Flag para evitar chamadas duplicadas de stop_all
