"""
Mecanismo de retentativa de chamadas API
Usado para lidar com lógica de retentativa de chamadas a APIs externas como LLM
"""
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if attempt == max_retries:
                        logger.error(f"Função {func.__name__} ainda falhou após {max_retries} retentativas: {str(e)}")
                        raise
...
def retry_with_backoff_async(
    ...
    """
    Versão assíncrona do decorador de retentativa
    """
    import asyncio
        ...
        async def wrapper(*args, **kwargs) -> Any:
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if attempt == max_retries:
                        logger.error(f"Função assíncrona {func.__name__} ainda falhou após {max_retries} retentativas: {str(e)}")
                        raise
...
class RetryableAPIClient:
    """
    Encapsulamento de cliente API com retentativa
    """
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if attempt == self.max_retries:
                    logger.error(f"Chamada API ainda falhou após {self.max_retries} retentativas: {str(e)}")
                    raise
                ...
                logger.warning(
                    f"Chamada API falhou na tentativa {attempt + 1}: {str(e)}, "
                    f"retentando em {current_delay:.1f}s..."
                )
    ...
    def call_batch_with_retry(
        ...
        """
        Chamada em lote com retentativa individual para cada item falho
        ...
        Returns:
            (lista de resultados bem-sucedidos, lista de itens falhos)
        """
        results = []
        failures = []
